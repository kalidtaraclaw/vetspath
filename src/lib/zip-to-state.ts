 ...
 * Maps 3-digit ZIP code prefixes to US state abbreviations
 * Covers all 50 states with focus on accuracy for TX, VA, NC
 */

// Map of 3-digit ZIP prefixes to state abbreviation
const ZIP_PREFIX_MAP: Record<string, string> = {
  ...
  '860': 'AZ', '861': 'AZ', '862': 'AZ', '863': 'AZ', '864': 'AZ',
  '865': 'AZ',
  ...
  '925': 'CA', '926': 'CA', '927': 'CA', '928': 'CA', '929': 'CA',
  ...
  '515': 'IA', '516': 'IA', '517': 'IA', '518': 'IA', '519': 'IA',
  ...
  '211': 'MD', '212': 'MD', '213': 'MD', '214': 'MD', '215': 'MD',
  ...
  // Nevada: 889-897
  '889': 'NV', '890': 'NV', '891': 'NV', '892': 'NV', '893': 'NV',
  '894': 'NV', '895': 'NV', '896': 'NV', '897': 'NV',
  ...
  // North Carolina: 270-289 (focus state)
  ...
  '985': 'WA', '986': 'WA', '987': 'WA', '988': 'WA', '989': 'WA',
  ...
  // Washington D.C.: 200-205
  '200': 'DC', '201': 'DC', '202': 'DC', '203': 'DC', '204': 'DC',
  '205': 'DC',
};

/**
 * State full name lookup by abbreviation
 */
...
/**
 * Convert a ZIP code (full or partial) to state abbreviation
 * Accepts formats: "75001", "750", or even "75001-1234"
 * @param zip - The ZIP code to look up
 * @returns State abbreviation (e.g., "TX") or null if not found
 */
 ...
 * @param stateCode - State abbreviation to check
...
/**
 * Resolve state from either ZIP code or state code
 * @param input - Either a 5-digit ZIP code or 2-letter state code
 * @returns State abbreviation or null if not found
 */
export function resolveState(input: string): string | null {
  if (!input) return null;

  // Try as state code first (2 chars)
  if (input.length <= 2) {
    const stateCode = input.toUpperCase();
